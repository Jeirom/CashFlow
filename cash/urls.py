...
urlpatterns = [
    ...
    path(
        "<int:pk>/delete/", CashFlowDeleteView.as_view(), name="delete"
    ),  # до этого момента урлы - CRUD CashFlow
    path(
        "manage/", manage_all, name="manage"
    ),  # вьюшка редактирования остальных моделей
    # Status URLs
    ...
    path(
        "subcategories/add/", SubcategoryCreateView.as_view(), name="subcategory_create"
    ),
    path(
